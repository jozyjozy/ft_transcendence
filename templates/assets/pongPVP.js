...
function initgame(){
	...
	function winScreen(player) {
		...
		if (player ==  "Player 2")
		{
			...
			ctx.font = '24px Arial';
			ctx.fillText('Player 1 Score: ' + player1_score, canvas.width / 2, canvas.height / 2 + 50);
		}
		else if (player ==  "Player 1")
		{
			ctx.fillText('Player 1 WIN', canvas.width / 2, canvas.height / 2);
			ctx.font = '24px Arial';
			ctx.fillText('Player 2 Score: ' + player2_score, canvas.width / 2, canvas.height / 2 + 50);
		}
	}
    ...
    document.addEventListener('keydown', function(e) {
        if (gameFinished == 0 && Ws.readyState ===  WebSocket.OPEN)
            {
        if (e.key === 'ArrowUp') {
            Ws.send(JSON.stringify({player: 'player2', action: 'up'}));
        } else if (e.key === 'ArrowDown') {
            Ws.send(JSON.stringify({player: 'player2', action: 'down'}));
        } else if (e.key === 'w') {
    ...
    document.addEventListener('keyup', function(e) {
        if (gameFinished == 0 && Ws.readyState ===  WebSocket.OPEN)
            {
        if (e.key === 'ArrowUp') {
            Ws.send(JSON.stringify({player: 'player2', action: 'stop_up'}));
        } else if (e.key === 'ArrowDown') {
            Ws.send(JSON.stringify({player: 'player2', action: 'stop_down'}));
        } else if (e.key === 'w') {
